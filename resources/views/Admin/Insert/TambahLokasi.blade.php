    ...
    <div class="container mx-auto mt-10" style="margin-top: 10%">
        <div class="w-full max-w-lg mx-auto">
            <div class="bg-white p-8 rounded-lg shadow-md p-6">
                <h2 class="text-2xl font-semibold mb-6 text-center p-6">Tambah Lokasi</h2>

                <form method="POST" action="{{ route('admin.lokasi.prosesTambahLokasi') }}">
                    @csrf
                    ...
                    <div class="mb-4">
                        <label for="Kota" class="block text-sm font-medium text-gray-600">Nama Kota</label>
                        <input placeholder="Ex : Kota Bogor" id="Kota" type="text" class="form-input mt-1 block w-full border p-2 rounded @error('Kota') border-red-500 @enderror" name="Kota" value="{{ old('Kota') }}" required autofocus>
                        @error('Kota')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
