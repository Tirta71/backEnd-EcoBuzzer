...
<div class="flex-none w-full max-w-full px-3">
    <div class="mt-4 mb-4">
        <a href="{{ route('admin.kategori.tambahKategori') }}" class="px-4 py-2 bg-white text-blue-500 rounded-lg hover:bg-red-600 transition duration-300">Tambah Data</a>
    </div>
    
    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
        <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="dark:text-white">Table Kategori</h6>
        </div>
                        ...
                        <tr>
                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                <div class="flex px-2 py-1">
                                    <div class="flex flex-col justify-center">
                                        <h6 class="mb-0 text-sm leading-normal dark:text-white">{{ $kt->NamaKategori }}</h6>
                                    </div>
                                </div>
                            </td>
                            <td class="p-2 align-middle bg-transparent border-b-0 whitespace-nowrap shadow-transparent relative group" id="row{{ $index + 1 }}">
                                ...
                                <div class="hidden absolute top-0  left-full bg-white border shadow-md p-2 space-y-2" id="actions-row{{ $index + 1 }}">
                                    <!-- Tombol Edit -->
                                    <a href="{{ route('admin.kategori.edit', ['KategoriID' => $kt->KategoriID]) }}" class="text-xs text-gray-600 hover:text-red-500 py-1 px-2 rounded-md transition duration-300">Edit</a>


                                    <!-- Tombol Delete -->
                                    <a href="{{ route('admin.kategori.delete', ['KategoriID' => $kt->KategoriID]) }}" class="text-xs text-gray-600 hover:text-red-500 py-1 px-2 rounded-md transition duration-300" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $kt->KategoriID }}').submit();">
                                        Delete
                                    </a>
                                    
                                    <form id="delete-form-{{ $kt->KategoriID }}" action="{{ route('admin.kategori.delete', ['KategoriID' => $kt->KategoriID]) }}" method="POST" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
